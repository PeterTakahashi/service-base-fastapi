...
async def test_get_wallet_transaction(
    wallet_transaction_service, user, wallet_transaction
):
    fetched_wallet_transaction = await wallet_transaction_service.get(
        user_id=user.id, wallet_transaction_id=wallet_transaction.id
    )
...
async def test_get_wallet_transaction_not_found(wallet_transaction_service):
    with pytest.raises(NoResultFound):
        await wallet_transaction_service.get(user_id=uuid4(), wallet_transaction_id=0)
