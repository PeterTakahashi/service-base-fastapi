...
async def test_list_characters_by_product(character_repository, character):
    characters = await character_repository.list_characters_by_product(
        product_id=character.product.id, limit=10, offset=0
    )
...
async def test_list_characters_by_product_with_name_filter(
    character_repository, character
):
    product = character.product
    characters = await character_repository.list_characters_by_product(
        product_id=product.id, limit=10, offset=0, name=character.name
    )
...
async def test_list_characters_sorted_by_product(
    character_repository, product_with_characters
):
    characters = await character_repository.list_characters_by_product(
        product_id=product_with_characters.id,
        limit=10,
        offset=0,
        sort_by="created_at",
        sort_order="desc",
...
async def test_character_exists(character_repository, character):
    exists = await character_repository.character_exists(
        product_id=character.product.id, name=character.name
    )
...
async def test_character_not_exists(character_repository, character):
    exists = await character_repository.character_exists(
        product_id=character.product.id, name="NonExisting Character"
    )
...
async def test_create_character(character_repository, product):
    character = await character_repository.create_character(
        name="New Character", product_id=product.id
    )
...
async def test_get_character(character_repository, character):
    fetched_character = await character_repository.get_character(
        product_id=character.product.id, character_id=character.id
    )
...
async def test_get_character_not_found(character_repository):
    fetched_character = await character_repository.get_character(
        product_id=9999999, character_id=99999999
    )
...
async def test_update_character(character_repository, character):
    updated_character = await character_repository.update_character(
        character=character, data={"name": "Updated Character"}
    )
