...
async def test_product_exists_true():
    async for session in get_async_session():
        ...
        repo = ProductRepository(session)
        exists = await repo.product_exists(user_id=str(user.id), title="Existing Product")
...
async def test_product_exists_false():
    async for session in get_async_session():
        ...
        repo = ProductRepository(session)
        exists = await repo.product_exists(user_id=str(user.id), title="NonExisting Product")
...
async def test_create_product():
    async for session in get_async_session():
        ...
        repo = ProductRepository(session)
        product = await repo.create_product(title="Created Product", user_id=str(user.id))
...
async def test_get_product_success():
    async for session in get_async_session():
        ...
        repo = ProductRepository(session)
        product = await repo.get_product(user_id=str(user.id), display_id=str(created_product.display_id))
...
async def test_soft_delete_product():
    async for session in get_async_session():
        user = await create_user(session)
        created_product = await create_product(session, user, title="Soft Delete Product")
