...
async def test_create_product_duplicate_title(
    auth_client: AsyncClient, product: dict, product_id: str
):
    title = product["title"]

    response = await auth_client.post("/products/", json={"title": title})
