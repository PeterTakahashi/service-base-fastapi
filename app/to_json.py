...
class Translation(BaseModel):
    ...
    class Config:
        ...
        json_encoders = {np.ndarray: lambda array: Translation.encode_background(array)}
    ...
    def to_bytes(self):
        ...
        angle_bytes = struct.pack(
            "f", float(self.angle) if isinstance(self.angle, int) else self.angle
        )
        prob_bytes = struct.pack("f", self.prob)
        fg = struct.pack(
            "3B", self.text_color.fg[0], self.text_color.fg[1], self.text_color.fg[2]
        )
        bg = struct.pack(
            "3B", self.text_color.bg[0], self.text_color.bg[1], self.text_color.bg[2]
        )
        text_bytes = struct.pack("i", len(self.text.items()))
        for key, value in self.text.items():
            text_bytes += struct.pack("I", len(key.encode("utf-8"))) + key.encode(
                "utf-8"
            )
            text_bytes += struct.pack("I", len(value.encode("utf-8"))) + value.encode(
                "utf-8"
            )
        background_bytes = (
            struct.pack("I", len(self.background.tobytes())) + self.background.tobytes()
        )
        return (
            coords_bytes
            + is_bulleted_list_byte
            + angle_bytes
            + prob_bytes
            + fg
            + bg
