...
class ProductService:
    ...
    async def list_products(self, user_id: str, limit: int = 10, offset: int = 0, title: Optional[str] = None) -> List[ProductRead]:
        results = await self.product_repository.list_products(user_id, limit, offset, title)
        products = []
    ...
    async def create_product(self, user_id: str, data: ProductCreate) -> ProductRead:
        ...
        product = await self.product_repository.create_product(title=data.title, user_id=user_id)
    ...
    async def update_product(self, user_id: str, product_id: str, data: ProductUpdate) -> ProductRead:
        product = await self.product_repository.get_product(user_id, product_id)
        if not product:
            raise HTTPException(
                status_code=404,
        ...
        updated_product = await self.product_repository.update_product(product, data.model_dump(exclude_unset=True))
